...
| `orchestration/brief_assembly_pipeline.py` | 确定性模板拼接器：将 7 个模块的 JSON 输出组装为 Markdown 任务书 |
| `graph_engine/` | LangGraph 并行执行引擎，支持 7 模块并行 + 组装汇聚 |
...
项目参数 (名称、特征)
    ...
    └── 并行模式 (--mode parallel)
        ...
        │  • LangGraph StateGraph 并行执行 7 个模块             │
...
```
         ...
         ▼       ▼       ▼       ▼       ▼       ▼       ▼
    ┌────────┐┌────────┐┌────────┐┌────────┐┌────────┐┌────────┐┌────────┐
    │design  ││central ││exhibi- ││special ││science ││public  ││business│
    │        ││_hub    ││tion    ││_theater││_edu    ││_service││_research│
    └────┬───┘└────┬───┘└────┬───┘└────┬───┘└────┬───┘└────┬───┘└────┬───┘
         │         │         │         │         │         │         │
         └─────────┴─────────┴────┬────┴─────────┴─────────┴─────────┘
                                  ▼
